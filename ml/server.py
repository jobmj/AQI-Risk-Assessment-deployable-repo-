...
def metrics():
    try:
        ...
        mape      = float(np.mean(abs_err / np.maximum(true_aqi, 1)))
        ...
        bias = "over-predicts" if errors.mean() > 5 else \
               "under-predicts" if errors.mean() < -5 else "is well-balanced"
        interp = (
